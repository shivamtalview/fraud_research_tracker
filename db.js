...
// mode: 1 = Red Team (proactive), 2 = Investigation (reactive)
// Carried over from the original artifact's hardcoded seedEntries so no
// existing research is lost when the tracker moves to a real database.
const seedEntries = [
  {
...
const insertStmt = db.prepare(`
  ...
  VALUES (@date, @mode, @clients, @title, @findings, @summary, @references_json)
`);

function toRowParams(e) {
  return {
    date: e.date,
    mode: e.mode,
    ...
    references_json: JSON.stringify(e.references)
  };
}
...
function seedIfEmpty() {
  ...
  const tx = db.transaction((rows) => {
    for (const e of rows) insertStmt.run(toRowParams(e));
  });
  tx(seedEntries);
}
...
function insertEntry(e) {
  const info = insertStmt.run(toRowParams(e));
  const row = db.prepare("SELECT * FROM entries WHERE id = ?").get(info.lastInsertRowid);
  return rowToEntry(row);
}

module.exports = { getAllEntries, insertEntry };
